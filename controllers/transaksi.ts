...
const insertTransaksi = (req: Request, res: Response, next: NextFunction) => {
            ...
            Query<IMySQLResult>(connection, query)
                .then((result) => {
                    if (tipe_transaksi == 3 || tipe_transaksi == 4){
                        logging.info(NAMESPACE, "barang", barang)
                    }
                    logging.info(NAMESPACE, `Transaksi with id ${result.insertId} inserted.`);
